...
export default function SearchByTypeToggle({
    isSideBarOpen,
}) {
    const { toggleSearchByType } = useSearchByTypeStore();

    return (
        <div className="flex items-center pl-1">
            <input
                className="mt-[0.3rem] h-3.5 w-8 appearance-none rounded-[0.4375rem] 
                bg-white/50 before:pointer-events-nonebg-white/40 before:absolute 
                ...
                checked:bg-gradient-to-br from-blue to-lightpurple 
                ...
                focus:after:rounded-full focus:after:content-[''] checked:focus:border-lightpurple 
                checked:focus:bg-lightpurple checked:focus:before:ml-[1.0625rem] 
                checked:focus:before:scale-100 
                checked:focus:before:shadow-[3px_-1px_0px_13px_#3b71ca] 
                checked:focus:before:transition-[box-shadow_0.2s,transform_0.2s]"
                type="checkbox"
                role="switch"
                onClick={() => toggleSearchByType()}
                id="flexSwitchCheckDefault" />
            {isSideBarOpen ?
                <label
                    className="inline-block pl-4 text-sm hover:cursor-pointer"
